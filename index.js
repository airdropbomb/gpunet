...
async function readProxies() {
  try {
    ...
    if (proxies.length === 0) console.log(chalk.yellow('File proxy.txt is empty. Continuing without proxy.'));
    return proxies;
  } catch (error) {
...
function getProxyAgent(proxy) {
  ...
  if (proxy.startsWith('http://') || proxy.startsWith('https://')) {
    ...
    console.log(chalk.yellow(`Unknown proxy type: ${proxy}.`));
    return new HttpsProxyAgent(proxy);
  }
}
...
async function getNonce(proxy) {
  ...
  try {
    ...
    spinner.succeed(chalk.greenBright(`Nonce Obtained: ${nonce}`));
    return { nonce, cookie };
  } catch (error) {
...
async function signAndVerify(wallet, nonce, initialCookie, proxy) {
  ...
  try {
    ...
    if (response.data === 'ok') {
      spinner.succeed(chalk.greenBright('Login Successful'));
      return updatedCookie;
    } else {
      throw new Error('Verification Failed');
    }
  } catch (error) {
    spinner.fail(chalk.redBright(`Login Failed: ${error.message}`));
    return null;
  }
}
...
async function getUserInfo(cookie, proxy) {
  ...
  try {
    ...
    spinner.succeed(chalk.greenBright('Account Info Retrieved'));
    return userData;
  } catch (error) {
    spinner.fail(chalk.redBright(`Failed to Get Account Info: ${error.message}`));
    return null;
  }
}
...
async function getTasks(cookie, proxy) {
  ...
  try {
    ...
    spinner.succeed(chalk.greenBright('Task List Retrieved'));
    return tasks;
  } catch (error) {
    spinner.fail(chalk.redBright(`Failed to Get Task List: ${error.message}`));
    return null;
  }
}

async function completeTask(category, taskId, taskName, cookie, proxy) {
  const spinner = ora(`Completing Task: ${taskName}...`).start();
  let verifyUrl;
  switch (category) {
    case 'Social':
  ...
  try {
    ...
    if (response.data.success) {
      spinner.succeed(chalk.greenBright(`Task "${taskName}" Completed`));
      return true;
    } else {
      throw new Error(response.data.message);
    }
  } catch (error) {
    spinner.fail(chalk.redBright(`Failed to Complete Task "${taskName}": ${error.message}`));
    return false;
  }
}

async function dailyCheckIn(cookie, proxy) {
  const spinner = ora('Processing Daily Check-In...').start();
  try {
    ...
    if (isToday) {
      spinner.succeed(chalk.yellowBright(`Already Checked In Today, Streak: ${streak}`));
    } else {
      spinner.succeed(chalk.greenBright(`Check-in Successful, Streak: ${streak}`));
    }
    return streak;
  } catch (error) {
    spinner.fail(chalk.redBright(`Check-in Failed: ${error.message}`));
    return null;
  }
}
...
async function getGPXPoints(cookie, proxy) {
  ...
  try {
    ...
    spinner.fail(chalk.redBright(`Failed to Get Points Info: ${error.message}`));
    return null;
  }
}
...
async function processAccount(privateKey, proxy) {
  try {
    ...
    for (const [category, taskList] of Object.entries(tasks)) {
      ...
      for (const task of taskList) {
        if (category === 'GpuNet' && [3, 4, 5].includes(task.id)) {
          console.log(chalk.yellowBright(` ➥ Skipping Task: ${task.description} - Requires Manual Verification`));
          continue;
        }
        if (!task.completed) {
          await completeTask(category, task.id, task.description, sessionCookie, proxy);
        } else {
          console.log(chalk.bold.grey(` ➥ Task Already Completed: ${task.description}`));
        }
      }
    }
    ...
    console.log(chalk.yellowBright(`\nDone Processing Wallet: ${shorten(wallet.address)}`));
  } catch (error) {
...
async function processWallets(privateKeys, proxies, useProxy) {
  ...
  console.log();
  console.log(chalk.bold.cyanBright('='.repeat(80)));
  console.log(chalk.greenBright('All Accounts Processed.\n'));
...
async function run() {
  ...
  console.log(centerText("✪ GPUNET Auto Daily Check-in & Task ✪ \n"));
  ...
  if (useProxy) {
    ...
    if (proxies.length === 0) {
      console.log(chalk.yellow('No Proxies Available in proxy.txt, Continuing Without Proxy.'));
    }
  }

  const privateKeys = await readPrivateKeys();
  if (privateKeys.length === 0) {
    console.log(chalk.red('No Private Keys Found in pk.txt. Exiting...'));
    return;
  }
